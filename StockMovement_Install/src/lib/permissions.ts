...
export const PERMISSION_LIST: PermissionItem[] = [
    ...
    { key: PERMISSIONS.ADMIN_PO, label: 'ใบสั่งซื้อ (PO)', description: 'จัดการ PO สินค้าทั่วไป', category: 'Admin' },
...
export const DEFAULT_PERMISSIONS: Record<string, RolePermissions> = {
    ...
    'manager': Object.fromEntries(PERMISSION_LIST.map(p => [p.key, true])), // Manager gets most, restrict later
    'technician': {
    ...
    'employee': {
        [PERMISSIONS.DASHBOARD]: true,
        [PERMISSIONS.BORROW]: true, // Maybe restrict to view only?
        [PERMISSIONS.MAINTENANCE]: true,
    }
};

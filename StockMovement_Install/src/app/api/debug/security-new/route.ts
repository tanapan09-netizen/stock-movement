...
export async function GET() {
    console.log('Debug NEW: Starting simulation');
    try {
        const body: any = {
            csrfEnabled: true,
        ...
        const promises = keysToSave.map(async key => {
            ...
            // value is string "5", typeof is string. JSON.stringify skipped.
            if (typeof value !== 'string') value = JSON.stringify(value);

            return prisma.tbl_system_settings.upsert({
                where: { setting_key: key },
                update: { setting_value: value, description: 'Debug NEW' },
                create: { setting_key: key, setting_value: value, description: 'Debug NEW' },
            });
        });

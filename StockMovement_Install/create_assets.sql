...
-- Attempt to update enum if it doesn't match, might fail if column not empty or different type, but harmless to try or just ignore if it fails
-- ALTER TABLE `tbl_purchase_orders` MODIFY COLUMN `status` ENUM('draft', 'pending', 'approved', 'ordered', 'partial', 'received', 'cancelled') DEFAULT 'draft';

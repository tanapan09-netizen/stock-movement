...
export async function getUpcomingPmPlans(limit: number = 5) {
    try {
        const plans = await prisma.tbl_pm_plans.findMany({
            where: { active: true },
            include: { tbl_rooms: { select: { room_code: true, room_name: true } } },
            orderBy: { next_run_date: 'asc' },
            take: limit
        });
        return { success: true, data: plans };
    } catch (error) {
        console.error('Error fetching upcoming PM plans:', error);
        return { success: false, error: 'Failed to fetch upcoming PM plans' };
    }
}
